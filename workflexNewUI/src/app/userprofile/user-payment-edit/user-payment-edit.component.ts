...
@Component({
  selector: 'app-user-payment-edit',
  templateUrl: './user-payment-edit.component.html',
  styleUrls: ['./user-payment-edit.component.scss']
})
export class UserPaymentEditComponent implements OnInit {
    ...
    if(this.basicInfoForm.valid) {
      ...
      this.userService.updateUserDetails(serializedForm).subscribe((res) => {
        ...
        if(!resJSON.body.response.status) {
          ...
          return this.router.navigate([CONSTANTS_CLASS.ROUTS.GIGWORKER_PROFILE], { queryParams: {tab: 'payment'}});
        }
      }, (err) => {
        this.spinner.hide();
        this.toastr.errorToastr(err.errors);
